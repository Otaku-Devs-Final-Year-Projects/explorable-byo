...
export default function CommunityPage() {
 ...
 setLoading(false);
    ...
    // Realtime: prepend any new posts from other users
 ...
 if (loading) return <div className="min-h-screen bg-hotel-cream flex items-center justify-center text-hotel-bronze">Loading Community...</div>;
	...
	const handleLike = async (postId: string) => {
		// Optimistic update
		setDiscussions(prev => prev.map(p => p.id === postId ? { ...p, likes: p.likes + 1 } : p));
		try {
					...
					<div className="bg-white p-6 shadow-sm border border-hotel-sand mb-8">
						{!currentUser ? (
							<p className="text-sm text-gray-400 text-center py-2">
								<a href="/login" className="text-hotel-bronze font-bold hover:underline">Log in</a> to start a discussion.
							</p>
						) : (
