    ...
    useEffect(() => {
        const fetchArticle = async () => {
            ...
            try {
                ...
                const allArticles = (data && data.length > 0) ? data : mockArticles;
                const found = allArticles.find((a: any) => slugify(a.title) === slug);
                setArticle(found || null);
            } catch (err) {
                console.log("Using mock data for article:", err);
                const found = mockArticles.find(a => slugify(a.title) === slug);
                setArticle(found || null);
            } finally {
                setLoading(false);
            }
