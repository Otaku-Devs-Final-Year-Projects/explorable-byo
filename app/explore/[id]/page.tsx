...
import SaveVenueButton from '../../components/ui/SaveVenueButton';
...
export default function VenueDetails() {
 ...
 if (!dateRange?.from) {
 setBookingStatus("error"); // missing dates
 return;
 }
 ...
 check_out_date: dateRange.to ? format(dateRange.to, 'yyyy-MM-dd') : format(dateRange.from, 'yyyy-MM-dd'),
 guest_count: guestCount,
 room_count: roomCount,
 special_requests: needs.join(', ')
 });
 ...
 if (venue.owner_id) {
     await supabase.from('notifications').insert({
         ...
         message: `You have a new requested stay starting ${format(dateRange.from, 'MMM do')}.`
     });
 }
 ...
 <div className="ml-4">
 <SaveVenueButton venueId={venue.id} />
 </div>
 </div>
 ...
 className="w-full bg-hotel-black text-white py-4 font-serif uppercase tracking-[0.2em] text-xs font-bold hover:bg-hotel-bronze transition-colors duration-300 mb-3"
 >
 {user ? 'Check Availability' : 'Log In to Book'}
 </button>
 ...
 <div className="relative bg-white w-full max-w-lg rounded-sm shadow-2xl animate-fade-in-up flex flex-col max-h-[90vh]">
 <div className="bg-hotel-black text-white p-6 flex justify-between items-center shrink-0">
 ...
 <label className="text-[10px] uppercase font-bold text-gray-500">Party Size</label>
 <div className="relative">
 ...
 <div className="space-y-1">
 <label className="text-[10px] uppercase font-bold text-gray-500">Rooms/Slots Needed</label>
 <div className="relative">
 <DoorOpen className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" size={14} />
 <select value={roomCount} onChange={(e) => setRoomCount(parseInt(e.target.value))} className="w-full bg-gray-50 border border-gray-200 pl-10 py-2 text-sm rounded-sm">
 ...
 <div>
 <label className="text-[10px] uppercase font-bold text-gray-500 block mb-2">Select Your Stay</label>
 <BookingCalendar venueId={venue.id} capacity={venue.capacity || 1} requestedSlots={roomCount} onSelectRange={setDateRange} />
 </div>
 </div>
