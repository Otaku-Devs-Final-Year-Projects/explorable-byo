-- ===================================================================================
-- EXPLORABLE BYO - TRUE NUCLEAR SCHEMA RESET & SUPER SEED (FINAL V5)
-- ===================================================================================
-- This script uses session overrides to bypass any sticky constraints, perfectly 
-- deleting all previous `@test.com` accounts, and re-injecting them flawlessly.
-- ===================================================================================

-- 1. FORCE OBLITERATE OLD DATA & SCHEMAS
-- -----------------------------------------------------------------------------------
-- Temporarily bypass all foreign keys and triggers to ensure deletions succeed globally
...
-- Nuke ALL test identities and users
DELETE FROM auth.identities WHERE provider = 'email' AND identity_data->>'email' LIKE '%@test.com';
...
(p1_id, 'authenticated', 'authenticated', 'partner1@test.com', crypt('password123', gen_salt('bf', 10)), now(), '{"provider":"email","providers":["email"]}', '{"full_name":"Rob (Partner 1)", "role":"partner"}', now(), now()),
(p2_id, 'authenticated', 'authenticated', 'partner2@test.com', crypt('password123', gen_salt('bf', 10)), now(), '{"provider":"email","providers":["email"]}', '{"full_name":"Sarah (Partner 2)", "role":"partner"}', now(), now()),
(p3_id, 'authenticated', 'authenticated', 'partner3@test.com', crypt('password123', gen_salt('bf', 10)), now(), '{"provider":"email","providers":["email"]}', '{"full_name":"David (Partner 3)", "role":"partner"}', now(), now()),
(p4_id, 'authenticated', 'authenticated', 'partner4@test.com', crypt('password123', gen_salt('bf', 10)), now(), '{"provider":"email","providers":["email"]}', '{"full_name":"Elena (Partner 4)", "role":"partner"}', now(), now()),
(g1_id, 'authenticated', 'authenticated', 'guest1@test.com', crypt('password123', gen_salt('bf', 10)), now(), '{"provider":"email","providers":["email"]}', '{"full_name":"Alice (Guest 1)", "role":"guest"}', now(), now()),
(g2_id, 'authenticated', 'authenticated', 'guest2@test.com', crypt('password123', gen_salt('bf', 10)), now(), '{"provider":"email","providers":["email"]}', '{"full_name":"Bob (Guest 2)", "role":"guest"}', now(), now()),
(g3_id, 'authenticated', 'authenticated', 'guest3@test.com', crypt('password123', gen_salt('bf', 10)), now(), '{"provider":"email","providers":["email"]}', '{"full_name":"Charlie (Guest 3)", "role":"guest"}', now(), now()),
(g4_id, 'authenticated', 'authenticated', 'guest4@test.com', crypt('password123', gen_salt('bf', 10)), now(), '{"provider":"email","providers":["email"]}', '{"full_name":"Diana (Guest 4)", "role":"guest"}', now(), now());

-- Insert Identities securely
INSERT INTO auth.identities (id, user_id, provider_id, identity_data, provider, last_sign_in_at, created_at, updated_at) VALUES
(gen_random_uuid(), p1_id, p1_id::text, format('{"sub":"%s","email":"%s","email_verified":true}', p1_id::text, 'partner1@test.com')::jsonb, 'email', now(), now(), now()),
(gen_random_uuid(), p2_id, p2_id::text, format('{"sub":"%s","email":"%s","email_verified":true}', p2_id::text, 'partner2@test.com')::jsonb, 'email', now(), now(), now()),
(gen_random_uuid(), p3_id, p3_id::text, format('{"sub":"%s","email":"%s","email_verified":true}', p3_id::text, 'partner3@test.com')::jsonb, 'email', now(), now(), now()),
(gen_random_uuid(), p4_id, p4_id::text, format('{"sub":"%s","email":"%s","email_verified":true}', p4_id::text, 'partner4@test.com')::jsonb, 'email', now(), now(), now()),
(gen_random_uuid(), g1_id, g1_id::text, format('{"sub":"%s","email":"%s","email_verified":true}', g1_id::text, 'guest1@test.com')::jsonb, 'email', now(), now(), now()),
(gen_random_uuid(), g2_id, g2_id::text, format('{"sub":"%s","email":"%s","email_verified":true}', g2_id::text, 'guest2@test.com')::jsonb, 'email', now(), now(), now()),
(gen_random_uuid(), g3_id, g3_id::text, format('{"sub":"%s","email":"%s","email_verified":true}', g3_id::text, 'guest3@test.com')::jsonb, 'email', now(), now(), now()),
(gen_random_uuid(), g4_id, g4_id::text, format('{"sub":"%s","email":"%s","email_verified":true}', g4_id::text, 'guest4@test.com')::jsonb, 'email', now(), now(), now());
